    ...
    <template #wrapper>
      <el-card class="box-card">
        ...
        <el-row :gutter="10" class="mb8">
          <el-col :span="1.5">
            <el-button
              v-permisaction="['ssotokens:ssotokens:add']"
              type="primary"
              icon="el-icon-plus"
              size="mini"
              @click="handleAdd"
          ...
          <el-col :span="1.5">
            <el-button
              v-permisaction="['ssotokens:ssotokens:edit']"
              type="success"
              icon="el-icon-edit"
              size="mini"
              :disabled="single"
          ...
          <el-col :span="1.5">
            <el-button
              v-permisaction="['ssotokens:ssotokens:remove']"
              type="danger"
              icon="el-icon-delete"
              size="mini"
              :disabled="multiple"
        ...
        <el-table v-loading="loading" :data="ssotokensList" @selection-change="handleSelectionChange">
...
<script>
import { addSsoToken, delSsoToken, getSsoToken, listSsoToken, updateSsoToken } from '@/api/ssotokens'
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true
      listSsoToken(this.addDateRange(this.queryParams, this.dateRange)).then(response => {
        this.ssotokensList = response.data.list
    ...
    handleUpdate(row) {
      ...
      const id =
                row.id || this.ids
      getSsoToken(id).then(response => {
        this.form = response.data
        this.open = true
        this.title = '修改SsoTokens'
        this.isEdit = true
      })
    },
    /** 提交按钮 */
      ...
      this.$refs['form'].validate(valid => {
        if (valid) {
          if (this.form.id !== undefined) {
            updateSsoToken(this.form).then(response => {
            ...
            addSsoToken(this.form).then(response => {
              if (response.code === 200) {
