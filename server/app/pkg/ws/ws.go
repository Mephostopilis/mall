...
func (c *Client) Read() {
	defer func() {
		...
		c.log.Infof("client [%s] disconnect", c.Id)
		if err := c.Socket.Close(); err != nil {
			c.log.Errorf("client [%s] disconnect err: %s", c.Id, err)
		}
	}()
	...
	for {
		...
		c.log.Info("client [%s] receive message: %s", c.Id, string(message))
		c.Message <- message
	}
}
...
func (c *Client) Write() {
	defer func() {
		c.log.Info("client [%s] disconnect", c.Id)
		if err := c.Socket.Close(); err != nil {
			c.log.Error("client [%s] disconnect er	r: %s", c.Id, err)
		}
	}()
	...
	for {
		...
		case message, ok := <-c.Message:
			...
			c.log.Info("client [%s] write message: %s", c.Id, string(message))
			err := c.Socket.WriteMessage(websocket.TextMessage, message)
			if err != nil {
				c.log.Error("client [%s] writemessage err: %s", c.Id, err)
			}
		}
	}
}
...
func (manager *Manager) Start() error {
	...
	for {
		...
		case client := <-manager.Register:
			manager.log.Infof("client [%s] connect", client.Id)
			manager.log.Infof("register client [%s] to group [%s]", client.Id, client.Group)
		...
		case client := <-manager.UnRegister:
			manager.log.Infof("unregister client [%s] from group [%s]", client.Id, client.Group)
			manager.Lock.Lock()
...
// 	if err != nil {
// 		log.Error("websocket connect error: %s", ctx.GetString("channel"))
// 		return
// 	}

// 	fmt.Println("token: ", ctx.Request.URL.Query()["token"])

// 	u, _ := uuid.NewUUID()
// 	client := &Client{
// 		Id:      u.String(),
// 		Group:   ctx.GetString("channel"),
// 		Socket:  conn,
