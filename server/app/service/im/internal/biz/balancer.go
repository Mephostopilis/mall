...
func (lb *LoadBalancer) Update(ins []*registry.ServiceInstance) {
	var (
		totalConns  int64
		totalWeight int64
	...
	if len(ins) == 0 || float32(len(ins))/float32(len(lb.nodes)) < 0.5 {
		lb.log.Errorf("load balancer update src:%d target:%d less than half", len(lb.nodes), len(ins))
		return
	}
	lb.nodesMutex.Lock()
	for _, in := range ins {
		lastTS, _ := strconv.ParseInt(in.Metadata["LastTs"], 10, 64)
		if old, ok := lb.nodes[in.ID]; ok && old.updated == lastTS {
			nodes[in.ID] = old
			...
			if err != nil {
				lb.log.Errorf("instance(%+v) strconv.ParseInt(weight:%s) error(%v)", in, meta[model.MetaWeight], err)
				continue
			}
			conns, err := strconv.ParseInt(meta[model.MetaConnCount], 10, 32)
			if err != nil {
				lb.log.Errorf("instance(%+v) strconv.ParseInt(conns:%s) error(%v)", in, meta[model.MetaConnCount], err)
				continue
			}
			nodes[in.ID] = &weightedNode{
				region:       meta[model.MetaRegion],
				hostname:     in.ID,
				fixedWeight:  weight,
				currentConns: conns,
				addrs:        in.Endpoints,
