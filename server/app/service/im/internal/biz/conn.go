...
func (l *Logic) Connect(c context.Context, server, cookie string, token []byte) (mid int64, key, roomID string, accepts []int32, hb int64, err error) {
	...
	if err = json.Unmarshal(token, &params); err != nil {
		l.log.Errorf("json.Unmarshal(%s) error(%v)", token, err)
		return
	}
	mid = params.Mid
	roomID = params.RoomID
	accepts = params.Accepts
	hb = int64(l.c.Node.Heartbeat.Seconds) * int64(l.c.Node.HeartbeatMax)
	if key = params.Key; key == "" {
		key = uuid.New().String()
	}
	if err = l.dao.AddMapping(c, mid, key, server); err != nil {
		l.log.Errorf("l.dao.AddMapping(%d,%s,%s) error(%v)", mid, key, server, err)
	}
	l.log.Infof("conn connected key:%s server:%s mid:%d token:%s", key, server, mid, token)
	return
}
...
func (l *Logic) Disconnect(c context.Context, mid int64, key, server string) (has bool, err error) {
	if has, err = l.dao.DelMapping(c, mid, key, server); err != nil {
		l.log.Errorf("l.dao.DelMapping(%d,%s) error(%v)", mid, key, server)
		return
	}
	l.log.Infof("conn disconnected key:%s server:%s mid:%d", key, server, mid)
	return
}
...
func (l *Logic) Heartbeat(c context.Context, mid int64, key, server string) (err error) {
	...
	if err != nil {
		l.log.Errorf("l.dao.ExpireMapping(%d,%s,%s) error(%v)", mid, key, server, err)
		return
	}
	if !has {
		if err = l.dao.AddMapping(c, mid, key, server); err != nil {
	...
	l.log.Infof("conn heartbeat key:%s server:%s mid:%d", key, server, mid)
	return
}
...
func (l *Logic) Receive(c context.Context, mid int64, proto *grpc.Proto) (err error) {
	l.log.Infof("receive mid:%d message:%+v", mid, proto)
	return
}
