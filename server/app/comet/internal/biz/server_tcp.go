...
func InitTCP(logger log.Logger, server *Server, addrs []string, accept int) (err error) {
	...
	for _, bind = range addrs {
		...
		// split N core accept
		for i := 0; i < accept; i++ {
			go acceptTCP(log, server, listener)
		}
	}
	return
}
...
func acceptTCP(log *log.Helper, server *Server, lis *net.TCPListener) {
	var (
		conn *net.TCPConn
		err  error
		r    int
	...
	for {
		...
		if err = conn.SetReadBuffer(int(server.c.Tcp.ReadBuf)); err != nil {
...
func serveTCP(log *log.Helper, s *Server, conn *net.TCPConn, r int) {
	var (
		// timer
		tr = s.round.Timer(r)
		rp = s.round.Reader(r)
...
func (s *Server) ServeTCP(conn *net.TCPConn, rp, wp *bytes.Pool, tr *xtime.Timer) {
	...
	trd = tr.Add(time.Duration(s.c.Protocol.HandshakeTimeout.AsDuration()), func() {
		conn.Close()
		s.log.Errorf("key: %s remoteIP: %s step: %d tcp handshake timeout", ch.Key, conn.RemoteAddr().String(), step)
	})
	...
	if p, err = ch.CliProto.Set(); err == nil {
		if ch.Mid, ch.Key, rid, accepts, hb, err = s.authTCP(ctx, rr, wr, p); err == nil {
			...
			b = s.Bucket(ch.Key)
			err = b.Put(rid, ch)
			s.log.Infof("tcp connnected key:%s mid:%d proto:%+v", ch.Key, ch.Mid, p)
		}
	}
	step = 2
	if err != nil {
		...
		wp.Put(wb)
		tr.Del(trd)
		s.log.Errorf("key: %s handshake failed error(%v)", ch.Key, err)
		return
	}
	trd.Key = ch.Key
	tr.Set(trd, hb)
	...
	for {
		...
		if p.Op == grpc.OpHeartbeat {
			...
			s.log.Infof("tcp heartbeat receive key:%s, mid:%d", ch.Key, ch.Mid)
			step++
		} else {
	...
	if err != nil && err != io.EOF && !strings.Contains(err.Error(), "closed") {
		s.log.Errorf("key: %s server tcp failed error(%v)", ch.Key, err)
	}
	b.Del(ch)
	tr.Del(trd)
	rp.Put(rb)
	...
	if err = s.Disconnect(ctx, ch.Mid, ch.Key); err != nil {
		s.log.Errorf("key: %s mid: %d operator do disconnect error(%v)", ch.Key, ch.Mid, err)
	}
	if white {
		whitelist.Printf("key: %s mid: %d disconnect error(%v)\n", ch.Key, ch.Mid, err)
	}
	s.log.Infof("tcp disconnected key: %s mid: %d", ch.Key, ch.Mid)
}
...
func (s *Server) dispatchTCP(conn *net.TCPConn, wr *bufio.Writer, wp *bytes.Pool, wb *bytes.Buffer, ch *Channel) {
	...
	for {
		...
		s.log.Infof("key:%s dispatch msg:%v", ch.Key, *p)
		switch p {
		...
		default:
			...
			s.log.Infof("tcp sent a message key:%s mid:%d proto:%+v", ch.Key, ch.Mid, p)
		}
		if white {
...
failed:
	...
	if err != nil {
		s.log.Errorf("key: %s dispatch tcp error(%v)", ch.Key, err)
	}
	conn.Close()
	wp.Put(wb)
...
func (s *Server) authTCP(ctx context.Context, rr *bufio.Reader, wr *bufio.Writer, p *grpc.Proto) (mid int64, key, rid string, accepts []int32, hb time.Duration, err error) {
	...
	if mid, key, rid, accepts, hb, err = s.Connect(ctx, p, ""); err != nil {
		s.log.Errorf("authTCP.Connect(key:%v).err(%v)", key, err)
		return
	}
	p.Op = grpc.OpAuthReply
	p.Body = nil
	if err = p.WriteTCP(wr); err != nil {
		s.log.Errorf("authTCP.WriteTCP(key:%v).err(%v)", key, err)
		return
	}
	err = wr.Flush()
	return
