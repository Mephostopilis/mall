...
func InitWebsocket(logger log.Logger, server *Server, addrs []string, accept int) (err error) {
	...
	for _, bind = range addrs {
		...
		// split N core accept
		for i := 0; i < accept; i++ {
			go acceptWebsocket(log, server, listener)
		}
	}
	return
}
...
func InitWebsocketWithTLS(log *log.Helper, server *Server, addrs []string, certFile, privateFile string, accept int) (err error) {
	var (
		bind     string
	...
	for _, bind = range addrs {
		...
		// split N core accept
		for i := 0; i < accept; i++ {
			go acceptWebsocketWithTLS(log, server, listener)
		}
	}
	return
}
...
func acceptWebsocket(log *log.Helper, server *Server, lis *net.TCPListener) {
	var (
		conn *net.TCPConn
		err  error
		r    int
	...
	for {
		...
		if err = conn.SetReadBuffer(int(server.c.Tcp.Rcvbuf)); err != nil {
			log.Errorf("conn.SetReadBuffer() error(%v)", err)
			return
		}
		if err = conn.SetWriteBuffer(int(server.c.Tcp.Sndbuf)); err != nil {
			log.Errorf("conn.SetWriteBuffer() error(%v)", err)
			return
		}
		go serveWebsocket(log, server, conn, r)
		if r++; r == maxInt {
			r = 0
		}
	}
...
// for each incoming connection.  Accept blocks; the caller typically
// invokes it in a go statement.
func acceptWebsocketWithTLS(log *log.Helper, server *Server, lis net.Listener) {
	var (
		conn net.Conn
		err  error
		r    int
...
func serveWebsocket(log *log.Helper, s *Server, conn net.Conn, r int) {
	var (
		// timer
		tr = s.round.Timer(r)
		rp = s.round.Reader(r)
...
func (s *Server) ServeWebsocket(conn net.Conn, rp, wp *bytes.Pool, tr *xtime.Timer) {
	var (
		...
		lastHB  = time.Now()
		rb      = rp.Get()
		ch      = NewChannel(int(s.c.Protocol.CliProto), int(s.c.Protocol.SvrProto))
		rr      = &ch.Reader
		wr      = &ch.Writer
	...
	// handshake
	step := 0
	trd = tr.Add(time.Duration(s.c.Protocol.HandshakeTimeout.AsDuration()), func() {
		// NOTE: fix close block for tls
		_ = conn.SetDeadline(time.Now().Add(time.Millisecond * 100))
		_ = conn.Close()
		s.log.Errorf("key: %s remoteIP: %s step: %d ws handshake timeout", ch.Key, conn.RemoteAddr().String(), step)
	})
	// websocket
	...
	if req, err = websocket.ReadRequest(rr); err != nil || req.RequestURI != "/sub" {
		...
		rp.Put(rb)
		if err != io.EOF {
			s.log.Errorf("http.ReadRequest(rr) error(%v)", err)
		}
		return
	}
	// writer
	...
	if ws, err = websocket.Upgrade(conn, rr, wr, req); err != nil {
		...
		wp.Put(wb)
		if err != io.EOF {
			s.log.Errorf("websocket.NewServerConn error(%v)", err)
		}
		return
	}
	...
	if p, err = ch.CliProto.Set(); err == nil {
		if ch.Mid, ch.Key, rid, accepts, hb, err = s.authWebsocket(ctx, ws, p, req.Header.Get("Cookie")); err == nil {
			...
			s.log.Infof("websocket connnected key:%s mid:%d proto:%+v", ch.Key, ch.Mid, p)
		}
	}
	step = 4
	if err != nil {
		...
		if err != io.EOF && err != websocket.ErrMessageClose {
			s.log.Errorf("key: %s remoteIP: %s step: %d ws handshake failed error(%v)", ch.Key, conn.RemoteAddr().String(), step, err)
		}
		return
	}
	trd.Key = ch.Key
	...
	for {
		...
		if p.Op == grpc.OpHeartbeat {
			...
			s.log.Infof("websocket heartbeat receive key:%s, mid:%d", ch.Key, ch.Mid)
			step++
		} else {
	...
	if err != nil && err != io.EOF && err != websocket.ErrMessageClose && !strings.Contains(err.Error(), "closed") {
		s.log.Errorf("key: %s server ws failed error(%v)", ch.Key, err)
	}
	b.Del(ch)
	tr.Del(trd)
	ws.Close()
	...
	if err = s.Disconnect(ctx, ch.Mid, ch.Key); err != nil {
		s.log.Errorf("key: %s operator do disconnect error(%v)", ch.Key, err)
	}
	if white {
		whitelist.Printf("key: %s disconnect error(%v)\n", ch.Key, err)
	}

	s.log.Infof("websocket disconnected key: %s mid:%d", ch.Key, ch.Mid)
}
...
func (s *Server) dispatchWebsocket(ws *websocket.Conn, wp *bytes.Pool, wb *bytes.Buffer, ch *Channel) {
	...
	s.log.Infof("key: %s start dispatch tcp goroutine", ch.Key)
	for {
		if white {
		...
		s.log.Infof("key:%s dispatch msg:%s", ch.Key, p.Body)
		switch p {
		case grpc.ProtoFinish:
			...
			s.log.Infof("key: %s wakeup exit dispatch goroutine", ch.Key)
			finish = true
			goto failed
		...
		default:
			...
			s.log.Infof("websocket sent a message key:%s mid:%d proto:%+v", ch.Key, ch.Mid, p)
		}
		if white {
...
failed:
	...
	if err != nil && err != io.EOF && err != websocket.ErrMessageClose {
		s.log.Errorf("key: %s dispatch ws error(%v)", ch.Key, err)
	}
	ws.Close()
	wp.Put(wb)
	...
	if s.Debug {
		s.log.Infof("key: %s dispatch goroutine exit", ch.Key)
	}
}
...
func (s *Server) authWebsocket(ctx context.Context, ws *websocket.Conn, p *grpc.Proto, cookie string) (mid int64, key, rid string, accepts []int32, hb time.Duration, err error) {
	for {
		...
		if p.Op == grpc.OpAuth {
			...
			s.log.Errorf("ws request operation(%d) not auth", p.Op)
		}
	}
